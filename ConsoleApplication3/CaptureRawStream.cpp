...
std::shared_ptr<pcl::visualization::PCLVisualizer> createPointCloudViewer(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud);
...
int generatePointCloud(rs::device *dev, pcl::PointCloud<pcl::PointXYZ>::Ptr);
...
int main(int argc, char** argv) try
{
	...
	// ==== Cloud Setup ====
	//creating a new PCL named rsCloudPtr
	...
	while (!pclVisualizer->wasStopped())
	{
		...
		/*		I'm not pretty sure why it couldn't work, although it seems logical
		if (command == 0)
		{
		pcl::io::savePCDFile("Realsense.pcd", *rsCloudPtr);
		return EXIT_SUCCESS;
		}
		*/
...
int generatePointCloud(rs::device *dev, pcl::PointCloud<pcl::PointXYZ>::Ptr rs_cloud_ptr)
{
